...
import { type AddictionType, ADDICTION_LABELS } from '@/src/models/tracker';
...
export default function CounterSetupScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-background" contentContainerClassName="p-6 pb-12">
      <Text className="text-2xl font-semibold mb-6">{t('counter.selectType')}</Text>
        ...
        {ADDICTION_TYPES.map((type) => (
          ...
          >
            <Text
              className={cn(
                'text-base',
                selectedType === type ? 'text-primary-foreground font-semibold' : 'text-foreground',
              )}
            >
      ...
      {selectedType === 'other' && (
        <View className="mt-4">
          <Text className="text-sm text-muted-foreground mb-2">{t('counter.customLabel')}</Text>
          <Input
            value={customLabel}
      ...
      <Button className="mt-8" onPress={handleCreate} disabled={!selectedType}>
